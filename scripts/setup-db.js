...
const DATABASE_URL = process.env.DATABASE_URL || 'postgresql://inkforge:inkforge@localhost:5432/inkforge';
...
CREATE INDEX IF NOT EXISTS idx_novels_user_id ON novels(user_id)`,
    ...
    for (const stmt of statements) {
      console.log('Executing:', stmt.substring(0, 40).replace(/\n/g, ' ') + '...');
    ...
    if (err.message.includes('ECONNREFUSED')) {
      console.error('PostgreSQL is not running. Please start PostgreSQL or use Docker Compose.');
    } else if (err.message.includes('28P01')) {
      console.error('Authentication failed. Check username/password.');
    } else if (err.message.includes('3D000')) {
